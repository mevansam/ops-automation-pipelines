...
mysql_user=$(opsman::get_product_credential cf- mysql_admin_credentials | jq -r .credential.value.identity)
mysql_password=$(opsman::get_product_credential cf- mysql_admin_credentials | jq -r .credential.value.password)
